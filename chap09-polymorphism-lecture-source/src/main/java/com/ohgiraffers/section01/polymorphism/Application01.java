...
public class Application01 {
    ...
    public static void main(String[] args) {
        ...
        * */
        ...
        /* 목차. 3. 토깽이 인스턴스 생성 후 메소드 호출 확인 */
        System.out.println("=============토깽이생성==============");
        토깽이 rabbit = new 토깽이();
        토깽이 rabbit1 = new 토깽이();
        rabbit. 먹기();
        rabbit. 달리기();
        ...
        /* 목차. 4. 토깽이 인스턴스 생성 후 메소드 호출 확인 */
        System.out.println("=============호랭이생성==============");
        호랭이 tiger = new 호랭이();
        호랭이 tiger1 = new 호랭이();
        tiger. 먹기();
        tiger. 달리기();
        ...
        /* 필기.
        *   토깽이랑 호랭이는 동물 클래스를 상속 받았다.
        *   따라서 토끼는 토끼 타입이기도 하면서 동물 타입니다.
        *   호랭이도 마찬가지로 호랭이 타입이기도 하면서 동물 타입이다.
        * */

        /* 목차. 5. 부모타입(동물) 으로 자식 인스턴스(호랭이, 토깽이) 주소값 저장 */
...
//        호랭이 t1 = new 동물();
        ...
        *       토깽이와 호랭이가 가지고 있는 고유한 기능은 동작시키지 못한다.
        *  */
//        a1. 점프();
//        a2. 물기();
        ...
        /* 목차. 7. 타입 형변환 확인 */
        /* 필기.
        *   객체별 고유한 기능(점프, 물어뜯기)를 동작시키기 위해서는 형변환을 통해 토꺵이와 호랭이로 변경해야
        *   메소드 호출이 가능하다.
...
//        ((호랭이)a1).물어뜯기(); 토깽이는 호랭이가 될 수 없다. RuntimeException 번역은 문제가 없지만, 프로그램 동작 시 오류 발생

        /* 목차. 8. instanceof 연산자 사용 */
        /* 필기. 레퍼런스 변수가 참조하는 실제 인스턴스가 원하는 타입과 맞는지 비교하는 연산자. */
        System.out.println("==========instanceof 확인==========");
        System.out.println("a1이 호랭이 타입인지 확인 : " + (a1 instanceof 호랭이));
        System.out.println("a1이 토꺵이 타입인지 확인 : " + (a1 instanceof 토깽이));
        ...
        *  */
        ...
        if(a1 instanceof 토깽이) {
            ((토깽이) a1).점프();
        }

        if(a1 instanceof 호랭이) {
            ((호랭이)a1).물어뜯기();
        }
        ...
        * */
        ...
        동물 a3 = (동물)new 토깽이();      // up-casting 명시적 형변환
        동물 a4 = new 토깽이();           // up-casting 묵시적 형변환

//        토깽이 r3 = a3;                  // down-casting 묵시적 형변환은 안됨.
        토깽이 r4 = (토깽이)a3;             // down-casting 명시적 형변환
